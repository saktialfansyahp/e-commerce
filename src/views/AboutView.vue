      ...
      <div class="pb-4 bg-white dark:bg-gray-900 mx-2 my-2">
        ...
        <div class="relative flex justify-between items-center">
          ...
          <button
            @click="navigate"
            type="button"
            class="text-gray-900 bg-white border border-gray-300 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
...
export default {
      ...
      let filter = this.phone
        .filter((product) => {
          ...
          const isMatchingCategory =
            product.Product.Subcategory.Category.category
              .toLowerCase()
              .includes(searchTerm);
          const isMatchingSub = product.Product.Subcategory.sub_category
            .toLowerCase()
            .includes(searchTerm);
          return (
            isMatchingName ||
  ...
  methods: {
    ...
    deleteProduct() {
      axios
        .delete(`/api/admin/product/${this.product_id}`)
        .then((response) => {
    ...
    navigate() {
      localStorage.setItem("history", this.$route.name);
      this.$store.state.history = this.$route.name;
      this.$router.push("/about/addproduct");
    },
    fetchData() {
      axios
        .get("/api/product")
